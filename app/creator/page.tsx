...
export default function CreatorPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const u = await api<{ ok: boolean; users?: UserRow[] }>("/api/admin/users");
  ...
  async function save() {
    ...
    setNote(res.ok ? "Сохранено. Ключи на сервере, в браузер не возвращаются." : "Не удалось сохранить.");
  }

  async function changeRole(id: string, role: string) {
    const res = await api<{ ok: boolean }>("/api/admin/role", { method: "POST", body: JSON.stringify({ userId: id, role }) });
    setNote(res.ok ? "Роль обновлена." : "Ошибка смены роли.");
  }

  const set = (k: keyof typeof form) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => setForm((f) => ({ ...f, [k]: e.target.value }));

  if (allowed === false) return <div className="empty">Доступ только у Создателя.</div>;
  ...
  return (
    <>
      ...
      <div className="grid">
        <div>
          <div className="card">
            <h3 className="card-h">Роли и доступы</h3>
            <div className="list tight">
              {users.map((u) => (
                <div className="role-row" key={u.id}>
                  <div><div className="row-t">{u.nickname}</div><div className="row-d">{u.email}</div></div>
                  <select className="role-select" defaultValue={u.role} onChange={(e) => changeRole(u.id, e.target.value)}>
                    {ROLES.map((r) => <option key={r}>{r}</option>)}
                    <option>Создатель</option>
                  </select>
                </div>
              ))}
              {users.length === 0 && <div className="empty">Пользователи появятся после регистрации.</div>}
            </div>
          </div>
        </div>
        <aside>
          <div className="card">
            <h3 className="card-h">API-ключи</h3>
