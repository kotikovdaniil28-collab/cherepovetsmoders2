...
async function deepseek(system: string, user: string, key: string) {
  const res = await fetch("https://api.deepseek.com/chat/completions", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${key}` }, body: JSON.stringify({ model: "deepseek-chat", temperature: 0.3, messages: [{ role: "system", content: system }, { role: "user", content: user }] }) });
  const data = await res.json(); return data?.choices?.[0]?.message?.content ?? "AI не ответил.";
}
async function gemini(system: string, user: string, key: string) {
  const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${key}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ systemInstruction: { parts: [{ text: system }] }, contents: [{ parts: [{ text: user }] }] }) });
  const data = await res.json(); return data?.candidates?.[0]?.content?.parts?.[0]?.text ?? "AI не ответил.";
}
...
export async function POST(req: Request) {
  ...
  if (body.mode === "chat") {
    ...
    return NextResponse.json({ ok: true, answer: await deepseek(system, String(body.message ?? ""), key) });
  }
  if (body.mode === "verdict") {
    ...
    try { parsed = JSON.parse(String(raw).replace(/```json|```/g, "").trim()); } catch { parsed = { verdict: "Норма", reason: String(raw).slice(0, 140) }; }
